...
export const AdminVenueDetails = ({
    venue,
    onBack,
}: AdminVenueDetailsProps) => {
    const facts = [
    ...
    return (
        <section className={styles['admin-venue-details']}>
            <header className={styles['admin-venue-details__header']}>
                <button
                    type='button'
                    onClick={onBack}
                    className={styles['admin-venue-details__back']}
                    aria-label='Вернуться к списку заявок'
                >
                    <ArrowLeftOutlined />
                </button>

                <h2 className={styles['admin-venue-details__title']}>
                    Информация о заявке
                </h2>
            </header>
