...
const getModerationQueue = http.get<
    never,
    never,
    ListResponse
>(`${BASE}/admin/venues`, async ({ request }) => {
    ...
    const size = sizeParam ? Math.min(100, Math.max(1, Number(sizeParam))) : 20;

    let filtered = mockDb.venues.map(toAdminVenue);
    if (statusParam && isModerationStatus(statusParam)) {
        filtered = filtered.filter((v) => v.moderationStatus === statusParam);
    }
...
const getVenueById = http.get<
    IdParam,
    never,
    ItemResponse
>(`${BASE}/admin/venues/:id`, async ({ request, params }) => {
    await delay(LATENCY_MS);
...
const verifyVenue = http.patch<
    IdParam,
    never,
    ItemResponse
>(`${BASE}/admin/venues/:id/verify`, async ({ request, params }) => {
    await delay(LATENCY_MS);
...
const rejectVenue = http.patch<
    IdParam,
    AdminModerationDecision,
    ItemResponse
>(`${BASE}/admin/venues/:id/reject`, async ({ request, params }) => {
    await delay(LATENCY_MS);
